...
$stmt = $conn->prepare("SELECT username, email, phone_number, address FROM user WHERE id = ?");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$stmt->bind_result($name, $email, $phone_number, $location);
$stmt->fetch();
$stmt->close();
  ...
  <form action="booking_confirmation.php" method="POST">
    ...
    <div class="form-row">
      ...
      <div class="form-group">
        ...
        <input type="tel" name="phone_number" value="<?= htmlspecialchars($phone_number); ?>" required pattern="^09\d{9}$">
      </div>
    </div>
